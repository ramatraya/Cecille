...
class Ion_auth_model extends CI_Model {
     ...
     * @author Erwin
     * */
    public function change_password_no_old($identity,  $new) {
        $query = $this->db->select('password, salt')
            ->where($this->identity_column, $identity)
            ->where($this->ion_auth->_extra_where)
            ->limit(1)
    ...
    /**
     * get_user_by_unique_field
     *
     * for checking fb_id or twitter_id or any unique field
     * @return object
     * @author erwinhandoko
     * */
    public function get_user_by_unique_field($field, $identity) {
        $this->db->where($this->tables['meta'] . '.' . $field, $identity);
        $this->db->limit(1);
     ...
     *
     * @return object
     * @author Phil Sturgeon | erwin handoko
     * */
    public function get_groups($mode='') {
        $res = $this->db->get($this->tables['groups'])->result();
        if ($mode == 'dropdown') {
            $dd = array();
            foreach ($res as $r) {
                $dd[$r->id] = $r->name;
