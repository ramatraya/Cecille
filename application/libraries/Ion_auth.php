...
class Ion_auth {
    ...
    public function facebook_login($fbid) {
        $row = $this->ci->ion_auth_model->get_user_by_socmed('fb_id',$fbid);
        if($row) {
            $this->ci->ion_auth_model->update_last_login($row->id);

            $group = $this->ci->ion_auth_model->get_group($row->group_id);
